...
class NeuralNetValueFunction(object):
    ...
    def __init__(self, ob_dim, ac_dim):
        ...
        wd_dict = {}
        h1 = tf.nn.elu(dense(X, 256, "h1", weight_init=U.normc_initializer(1.0), bias_init=0, weight_loss_dict=wd_dict))
        h2 = tf.nn.elu(dense(h1, 256, "h2", weight_init=U.normc_initializer(1.0), bias_init=0, weight_loss_dict=wd_dict))
    ...
    def fit(self, paths, targvals):
        ...
        logger.record_tabular(u"EVBefore", common.explained_variance(self._predict(X), y))

        for _ in xrange(25):
            self.do_update(X, y)

        logger.record_tabular(u"EVAfter", common.explained_variance(self._predict(X), y))


def pathlength(path):
    return path[u"reward"].shape[0]
