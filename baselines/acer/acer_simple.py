...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, nstack, num_procs,
        ...
        MU = tf.placeholder(tf.float32, [nbatch, nact])  # m's
        ...
        # strip off last step
        f, f_pol, q = imap(lambda var: strip(var, nenvs, nsteps), [train_model.pi, polyak_model.pi, train_model.q])
...
def learn(policy, env, seed, nsteps=20, nstack=4, total_timesteps=int(80e6), q_coef=0.5, ent_coef=0.01,
    ...
    for acer.steps in xrange(0, total_timesteps, nbatch):
