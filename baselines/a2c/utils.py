...
def conv(x, scope, **_3to2kwargs):
    if 'init_scale' in _3to2kwargs:
        init_scale = _3to2kwargs['init_scale']; del _3to2kwargs['init_scale']
    else:
        init_scale = 1.0
    if 'pad' in _3to2kwargs:
        pad = _3to2kwargs['pad']; del _3to2kwargs['pad']
    else:
        pad = u'VALID'
    stride = _3to2kwargs['stride'];
    del _3to2kwargs['stride']
    rf = _3to2kwargs['rf'];
    del _3to2kwargs['rf']
    nf = _3to2kwargs['nf'];
    del _3to2kwargs['nf']
    with tf.variable_scope(scope):
        ...
        w = tf.get_variable(u"w", [rf, rf, nin, nf], initializer=ortho_init(init_scale))
        b = tf.get_variable(u"b", [nf], initializer=tf.constant_initializer(0.0))
        return tf.nn.conv2d(x, w, strides=[1, stride, stride, 1], padding=pad) + b


def fc(x, scope, nh, **_3to2kwargs):
    if 'init_bias' in _3to2kwargs:
        init_bias = _3to2kwargs['init_bias']; del _3to2kwargs['init_bias']
    else:
        init_bias = 0.0
    if 'init_scale' in _3to2kwargs:
        init_scale = _3to2kwargs['init_scale']; del _3to2kwargs['init_scale']
...
def check_shape(ts, shapes):
    ...
    for (t, shape) in izip(ts, shapes):
        assert t.get_shape().as_list() == shape, u"id " + unicode(i) + u" shape " + unicode(t.get_shape()) + unicode(
            shape)
        i += 1
