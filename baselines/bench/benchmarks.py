...
def register_benchmark(benchmark):
    ...
    if u'tasks' in benchmark:
        for t in benchmark[u'tasks']:
            if u'desc' not in t:
                t[u'desc'] = remove_version_re.sub(u'', t[u'env_id'])
...
register_benchmark({
    ...
    u'tasks': [{u'desc': _game, u'env_id': _game + _ATARI_SUFFIX, u'trials': 2, u'num_timesteps': int(10e6)} for _game in _atari7]
})
...
_mujocosmall = [
    u'InvertedDoublePendulum-v1', u'InvertedPendulum-v1',
    u'HalfCheetah-v1', u'Hopper-v1', u'Walker2d-v1',
    u'Reacher-v1', u'Swimmer-v1']
register_benchmark({
    u'name': u'Mujoco1M',
    u'description': u'Some small 2D MuJoCo tasks, run for 1M timesteps',
    u'tasks': [{u'env_id': _envid, u'trials': 3, u'num_timesteps': int(1e6)} for _envid in _mujocosmall]
})
register_benchmark({
