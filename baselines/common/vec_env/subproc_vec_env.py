...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == u'step':
            ...
            ob = env.reset_state()
            remote.send(ob)
        elif cmd == u'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == u'close':
            remote.close()
            break
        elif cmd == u'get_spaces':
...
class SubprocVecEnv(VecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        self.closed = False
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = izip(*[Pipe() for _ in xrange(nenvs)])
        ...
        self.remotes[0].send((u'get_spaces', None))
