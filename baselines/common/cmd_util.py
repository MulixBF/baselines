...
def make_atari_env(env_id, num_env, seed, wrapper_kwargs=None, start_index=0):
    ...
    def make_env(rank): # pylint: disable=C0111
        def _thunk():
            ...
            env.seed(seed + rank)
            env = Monitor(env, logger.get_dir() and os.path.join(logger.get_dir(), unicode(rank)))
            return wrap_deepmind(env, **wrapper_kwargs)
        return _thunk
    set_global_seeds(seed)
    return SubprocVecEnv([make_env(i + start_index) for i in xrange(num_env)])
...
def mujoco_arg_parser():
    ...
    """
    parser = arg_parser()
    parser.add_argument(u'--env', help=u'environment ID', type=unicode, default=u"Reacher-v1")
    parser.add_argument(u'--seed', help=u'RNG seed', type=int, default=0)
